    ...
    <div class="container">
      ...
      <div class="blog-info">
        ...
        <div class="upload-file">
          <label for="blog-photo">Завантажити обкладинку</label>
      ...
      <div class="blog-actions">
        <button @click="updateBlog">Зберегти зміни</button>
        <router-link class="router-button" :to="{ name: 'NewsPreview' }">Переглянути зміни</router-link>
      </div>
    </div>
  </div>
</template>
...
import { getNewsById, uploadCoverPhoto, updateNews } from '../api/news.service'
...
export default {
  ...
  async mounted() {
    await getNewsById(this.$route.params.newsId).then(data => {
      this.$store.commit('setCurrentNews', data)
    })
  },
  methods: {
    fileChange() {
    ...
    async updateBlog() {
      if (this.newsTitle.length !== 0 && this.newsText.length !== 0) {
        if (this.file) {
          ...
          await uploadCoverPhoto(this.file).then(async data => {
            const { fileUrl } = data
            ...
            await updateNews(this.$route.params.newsId, postData)
              .then(data => {
                this.loading = false
